...
<div class="general">
	...
	<li>기억클래스 지정</li>
		...
		<li class="general_sub2"><span class="blue">[Storage class]</span> Type 변수명; &nbsp;&nbsp;&nbsp;---&gt; ex) <span class="blue">auto</span> int num;</li>
                <li class="general_sub2">[Storage class] 생략 시 bloack 내에서는 auto 로, block 밖에서는 extern 으로 인식</li>
		<li class="general_sub2" id="list">* 기억클래스의 default 는 auto 이며, block 밖에서는 원래 쓰지 않는다(꼭 생략해야 한다)</li>
            </ul><br>
	<li>실행파일과 메모리 영역</li>
	    <ul>
		<li class="general_sub2">C 언어로 작성한 실행파일이 Memory(RAM) 내에서 실행될 때에는 Code 영역에 실행파일이 올라가고, Data 영역에<br>프로그램 내에서 선언된 변수나 동적메모리 할당된 기억공간이 잡히게 된다.</li>
            </ul>
	...
	<li>extern 기억클래스</li>
		...
		<li class="general_sub2" id="list">하나의 프로그램이 여러 개의 소스 파일로 이루어져 있을 때 각 소스 파일에서 extern 변수를 사용하려면 <span class="blue">하나의<br>파일에서 정의하고 나머지 파일에서는 선언</span><span class="red">(분할 컴파일러에 의해 컴파일 되므로 선언이 없으면 모른다)</span>해 주어야 한다.</li>
                   <ul>
                       ...
                       <li class="general_sub2 red" id="list">3) 여러 번 선언 가능(쓰기 전에는 위치 상관 없으며 block 안에서 선언 시 유효 범위는 그 block 안)</li>
		...
		<li class="general_sub2" id="list">cf) 컴파일러</li>
                   <ol> 
                       <li class="general_sub2">통합(구 방식) : object 파일 무조건 1개(한 개의 소스 파일 오류 시 다함께 다시 컴파일 해야 한다)</li>
                       <li class="general_sub2">분할(신 방식) : object 파일이 소스 파일 갯수 만큼</li>
                   </ol><br>
		<li class="general_sub2" id="list">cf) 이 경우 a.cpp 의 선언부가 정의부가 된다(다른 곳도 초기화 해 주면 정의부 중복 되어 error)</li>
                   <img src="<?=base_url()?>asset\lib\img\tutorial\storage_class\5.PNG" width=600px; height=100px; style="margin-left:30px"><br><br>
	    </ul>
	<li>static 기억클래스</li>
	    <ul>
